 ...
 * trigger uses the default "smart policy" misfire instruction, which causes the
 * trigger to advance to its next fire time (skipping those that it has missed)
 * - so that it does not refire immediately, but rather at the next scheduled
 * time.
 * </p>
 * 
...
public class InterruptExample {
  ...
  public void run() throws Exception {
    ...
    JobDetail job = newJob(DumbInterruptableJob.class).withIdentity("interruptableJob1", "group1")
        .build();
