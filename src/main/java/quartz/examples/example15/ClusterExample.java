  ...
  public void run(boolean inClearJobs, boolean inScheduleJobs) throws Exception {
    ...
    if (inScheduleJobs) {
      ...
      JobDetail job = newJob(SimpleRecoveryJob.class).withIdentity("job_" + count, schedId) // put
                                                                                            ...
                                                                                            // from
                                                                                            // where
          .requestRecovery() // ask scheduler to re-execute this job if it was in progress when the
                             // scheduler went
                             // down...
          .build();
      ...
      job = newJob(SimpleRecoveryJob.class).withIdentity("job_" + count, schedId) // put triggers in
                                                                                  // group named
                                                                                  // after
                                                                                  // the cluster
                                                                                  // node instance
